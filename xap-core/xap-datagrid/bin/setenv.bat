...
rem * For more information see http://docs.gigaspaces.com/xap/12.3/started/common-environment-variables.html *
rem ***********************************************************************************************************
...
set XAP_OPTIONS=-Djava.util.logging.config.file="%XAP_LOGS_CONFIG_FILE%" -Djava.security.policy="%XAP_SECURITY_POLICY%" -Djava.rmi.server.hostname="%XAP_NIC_ADDRESS%" -Dcom.gs.home="%XAP_HOME%"

if not defined JAVA_OPTIONS (
	pushd "%XAP_HOME%\lib\required"
	FOR /F "tokens=*" %%i IN ('%JAVACMD% --illegal-access=permit -cp xap-datagrid.jar com.gigaspaces.internal.utils.OutputJVMOptions') DO set JAVA_OPTIONS=%%i %EXT_JAVA_OPTIONS%
	popd
)
